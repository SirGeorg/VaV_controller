...
void loop() {
    ...
    if (now - lastDampersMs >= 10000) {
        ...
        lastDampersMs = now;
        // Заслонки всегда вызываются: в сервисном режиме они лишь удерживают позицию,
        // в обычном — рассчитывают цели по CO2/фрикулингу/ручному режиму.
        dampers.update(dt, powerOn, serviceMode);
        if (!serviceMode) {
