...
void MqttManager::handleCommand(const String &topic, const String &payload) {
    ...
    if (sub == "set/restart") { 
        if (payload.toInt() != 0) {
            eventLog.add("Restart requested via MQTT");
            esp_restart();
        }
        return; 
    }
...
void MqttManager::publishState() {
...

    {
        ...
        doc["dp_ki"] = fan.dpKi();
        doc["dp_kd"] = fan.dpKd();
        doc["min_fan_step"] = storage.getMinFanStep();
        doc["min_fan_speed"] = storage.getMinFanSpeed();
        doc["min_dp_step"] = storage.getMinDpStep();
        doc["dp_within_setpoint_pct"] = storage.getDpWithinSetpointPct();
        doc["dp_low_timeout"] = storage.getDpLowTimeout();
        doc["dp_high_timeout"] = storage.getDpHighTimeout();
        doc["dp_alarm_low"] = storage.getDpAlarmLow();
        doc["dp_alarm_high"] = storage.getDpAlarmHigh();
        doc["flow_pct"] = sensors.flowPct();
        doc["flow_sensor_ok"] = sensors.flowSensorOk();
        doc["flow_alarm_threshold"] = storage.getFlowAlarmThreshold();
...

    {
        ...
        doc["heater_deadband_low"] = storage.getHeaterDeadbandLow();
        doc["heater_deadband_high"] = storage.getHeaterDeadbandHigh();
        doc["heater_alarm_temp"] = storage.getHeaterAlarmTemp();
        doc["heater_min_fan_runtime"] = storage.getHeaterMinFanRuntime();
        String out; serializeJson(doc, out);
        publishRetained(root_ + "/heater/state", out);
    }

    // General state with additional settings
    {
        DynamicJsonDocument doc(512);
        doc["power"] = powerOn_;
        doc["winter_mode"] = storage.getWinterMode();
        ...
        doc["uptime_s"] = millis() / 1000;
        doc["outdoor_temp"] = sensors.outdoorTempValid() ? sensors.outdoorTemp() : NAN;
        doc["co2_deadband"] = storage.getCo2Deadband();
        doc["co2_alarm_threshold"] = storage.getCo2AlarmThreshold();
        doc["min_servo_step"] = storage.getMinServoStep();
...
void MqttManager::publishHaDiscovery() {
    ...
    // Используем clientId_ как идентификатор устройства для уникальности
    String deviceId = clientId_;
    String deviceName = clientId_;  // Имя устройства из Client ID MQTT
    ...
    auto makeDeviceJson = [&](DynamicJsonDocument& doc) {
        ...
        dev["identifiers"] = deviceId.c_str();
        dev["name"] = deviceName.c_str();
    ...
    pubSensor("heater_fault_code", "Heater Fault Code", root + "/heater/state", "{{ value_json.fault_code }}", "");
    pubSensor("heater_fault_temp_at_trip", "Heater Fault Temp At Trip", root + "/heater/state", "{{ value_json.fault_temp_at_trip }}", "°C");
